...
from model.electra_module import Disc_only
...
if args.load_weights != None:
    if 'matbert' in args.load_weights:
        print("matbert!")
        tokenizer = AutoTokenizer.from_pretrained(args.load_weights, **tokenizer_kwargs)
    else:
        tokenizer = AutoTokenizer.from_pretrained(model_name, **tokenizer_kwargs)
else:
    tokenizer = AutoTokenizer.from_pretrained(model_name, **tokenizer_kwargs)
...
for lr in args.lm_lrs:
    ...
    for SEED in args.seeds:
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
                "/home/user10/MatSciBERT/baseline/DAS_100000",
                from_tf=False,
                config=config,
                cache_dir=cache_dir,
                revision=model_revision,
        ...
        '''
        #model.load_state_dict(torch.load("/home/user10/MatSciBERT/model/MatSciBert_random_mlm_120000.pt"), strict = False)
        if args.load_weights != None:
            text = args.load_weights[-4:]
...
file_path = os.path.join(preds_save_dir, f'glass_non_glass/{args.model_name}.txt')

with open(file_path, "a") as f:
    f.write(f'{args.model_name}_{args.load_weights} : {best_val} | {np.mean(best_test_acc_list)} | {best_val_acc_list} | {best_test_acc_list}\n')