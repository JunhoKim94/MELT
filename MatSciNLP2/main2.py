...
class Args(object):
    def __init__(self):
        ...
        parser.add_argument('--explanation',default=True,action='store_true')
        parser.add_argument('--setting',default='low_resource',type=str)
        parser.add_argument('--train_size',default=0.01,type=float)
        parser.add_argument('--basemodel',default='bert',type=str)
        parser.add_argument('--explanation_type',default='schema',type=str)
        #parser.add_argument('--explanation_type', default='schema', type=str, choices=['choice', 'schema', 'example'])
...
def test(model, dataloader, criterion, vocab_size, res_dict):
    ...
    y_pred = defaultdict(list)
    with torch.no_grad():
        for k, batch in enumerate(tqdm(dataloader, desc="Testing")):
...
if __name__ == "__main__":
    ...
    if (BASE_MODEL=='matscibert'):
        ...
        params['encoder'] = AutoModel.from_pretrained("/home/user25/Work_Space/Material_aware_LM/NLP4MatSci-ACL23/matbert-base-uncased")
        ...
        params['encoder'] = AutoModel.from_pretrained('google-bert/bert-base-uncased')
        
    elif (BASE_MODEL=='ourbert_base'):
        ...
        model_name_temp= 'm3rg-iitd/matscibert'
        config = AutoConfig.from_pretrained(model_name_temp, **config_kwargs)
        model_name = "/home/user25/Work_Space/Material_aware_LM/BERT/checkpoint-100000/pytorch_model.bin"
        
        #params['encoder']  = BertForMaskedLM.from_pretrained(model_name, config=config)
        params['encoder']  = AutoModel.from_pretrained(model_name, config=config)    

    elif (BASE_MODEL=='ourmatscibert_base'):
        # Load the model configuration from the local file
    ...
    elif (BASE_MODEL=='our_masked_matscibert'):
        # Load the model configuration from the local file
        ...
        model_name_temp= 'm3rg-iitd/matscibert'
        config = AutoConfig.from_pretrained(model_name_temp, **config_kwargs)
        model_name = "/home/user10/MatSciBERT/model/all/0.4/chem_matscivec_embedding_mat2vec_masking_curriculum_random_128/MatSciBert_concept_mlm_fixed_phrase_128_100000_curri_3.pt"
        
        #params['encoder']  = BertForMaskedLM.from_pretrained(model_name, config=config)
        params['encoder']  = AutoModel.from_pretrained(model_name, config=config)
    elif (BASE_MODEL=='our_masked_matscibert_0610'):
        # Load the model configuration from the local file
        ...
        model_name_temp= 'm3rg-iitd/matscibert'
        config = AutoConfig.from_pretrained(model_name_temp, **config_kwargs)
        model_name = "/home/user25/Work_Space/Material_aware_LM/error_MatSciBERT/11-result/MatSciBert_concept_mlm_fixed_phrase_128_100000_curri_3_extra_0.200000_total_0.400000_raw_0.200000.pt"
        
        #params['encoder']  = BertForMaskedLM.from_pretrained(model_name, config=config)
        params['encoder']  = AutoModel.from_pretrained(model_name, config=config)
        print(params['encoder'])
    elif (BASE_MODEL=='difference_masking'):
        # Load the model configuration from the local file
        ...
        model_name_temp= 'm3rg-iitd/matscibert'
        config = AutoConfig.from_pretrained(model_name_temp, **config_kwargs)
        model_name = "/home/user25/Work_Space/Material_aware_LM/error_MatSciBERT/11-result/v3_difference_masking_mlm_128_whole_dataset_100000.pt"
        ...
        model_name_temp= 'm3rg-iitd/matscibert'
        config = AutoConfig.from_pretrained(model_name_temp, **config_kwargs)
        model_name = "/home/user25/Work_Space/Material_aware_LM/error_MatSciBERT/11-result/100000_das/pytorch_model.bin"
        #model_name = "/home/user25/Work_Space/Material_aware_LM/error_MatSciBERT/MatSciBERT/ner/model/MatSciBERT/checkpoint-100000/pytorch_model.bin"
        
        #params['encoder']  = BertForMaskedLM.from_pretrained(model_name, config=config)
        params['encoder']  = AutoModel.from_pretrained(model_name, config=config)    

    elif (BASE_MODEL=='matkg'):
        # Load the model configuration from the local file
        ...
        model_name_temp= 'm3rg-iitd/matscibert'
        config = AutoConfig.from_pretrained(model_name_temp, **config_kwargs)
        model_name = "/home/user10/MatSciBERT/model/matkg_128_batch_128/MatSciBert_concept_mlm_fixed_phrase_128_100000_curri_3_extra_0.000000_total_0.300000_raw_0.150000.pt"
        #model_name = "/home/user25/Work_Space/Material_aware_LM/error_MatSciBERT/MatSciBERT/ner/model/MatSciBERT/checkpoint-100000/pytorch_model.bin"
