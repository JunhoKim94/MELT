            ...
            if (v!="O"):
                ...
                if (explanation==0):
                    ...
                    if (explanation_type=='choice'):
                        ...
                        candidates = res_dict['t_type_set']
                        exp_text = 'choices : ' + ','.join(candidates)
                        question_list.append('named entity recognition \t {}.  1. schema: <entity type> 2. choices {}. 3. example: text: {} named entity recognition \t {} answer: {}'.format(k, exp_text,exp_sent, exp_token, exp_label))
                        ...
                        #question_list.append('task: named entity recognition \t {}. text: {} named entity recognition \t {} answer: {}'.format(k, exp_sent, exp_token, exp_label))
            ...
            if (l!="O"):
                ...
                if (explanation==0):
                    ...
                    if (explanation_type=='choice'):
                        ...
                        candidates = res_dict['t_type_set']
                        exp_text = 'choices : ' + ','.join(candidates)
                        question_list.append('named entity recognition \t {}.  1. schema: <entity type> 2. choices {}. 3. example: text: {} named entity recognition \t {} answer: {}'.format(t, exp_text,exp_sent, exp_token, exp_token_label))
            ...
            if (s!="O"):
                ...
                if (explanation==0):
                    ...
                    if (explanation_type=='choice'):
                        ...
                        candidates = res_dict['sf_type_set']
                        exp_text = 'choices : ' + ','.join(candidates)
                        question_list.append('slot filling \t {}.  1. schema: <slot text> 2. choices {}. 3. example: text: {} slot filling \t {} answer: {}'.format(t, exp_text,exp_sent, exp_token, exp_token_label))
        ...
        for key,value in item['t_span_dict'].items():
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    candidates = res_dict['t_type_set']
                    exp_text = 'choices : ' + ','.join(candidates)                    
                    question_list.append('named entity recognition \t {}.  1. schema: <entity type> 2. choices {}. 3. example: {} named entity recognition \t {} answer: {}'.format(entity_name, exp_text, exp_text, exp_entity_name, exp_entity_type))
                    #question_list.append('slot filling \t {}.  1. schema: <slot text> 2. choices {}. 3. example: text: {} slot filling \t {} answer: {}'.format(t, exp_text,exp_sent, exp_token, exp_token_label))
        ...
        for key,value in item['r_args_dict'].items():
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    exp_text = ' choices : ' + ','.join(candidates) 
                    try: 
                        question_list.append('relation classification' + '\t' + ','.join(args)  + '1. schema: <relation type> 2. choices' + exp_text + f' 3. example: {exp_text} relation classification \t {exp_args[0]} {exp_args[1]} answer: {exp_relation_type}')
                    except:
                        question_list.append('relation classification' + '\t' + ','.join(args)  + '1. schema: <relation type> 2. choices' + exp_text )
        
                    #question_list.append('named entity recognition \t {}.  1. schema: <entity type> 2. choices {}. 3. example: text: {} named entity recognition \t {} answer: {}'.format(entity_name, exp_text, exp_text, exp_entity_name, exp_entity_type))
        ...
        for key,value in item['e_trig_dict'].items():
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    candidates = res_dict['e_role_set']
                    exp_text = ' choices : ' + ','.join(candidates)
                    question_list.append(f'event extraction \t {trigger}. 1. schema: argument mask:role mask 2. {exp_text}. 3. example: {exp_text} event extraction \t {exp_trigger} answer: {",".join(exp_event_struc)}')

                    #question_list.append(f'event extraction \t {trigger}. 1. schema: argument mask:role mask 2. choices {exp_text}. 3. example: {exp_text} event extraction \t {exp_trigger} answer: {','.join(exp_event_struc)}')
        ...
        for key,value in item['t_span_dict'].items():
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    candidates = res_dict['t_type_set']
                    exp_text = 'choices : ' + ','.join(candidates)
                    question_list.append(f'named entity recognition \t {entity_name}. 1. schema: <entity type> 2. {exp_text}. 3. example: {exp_text} named entity recognition \t {exp_entity_name} answer: {exp_entity_type}')
        ...
        for key,value in item['r_args_dict'].items():
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    try:
                        question_list.append(f'relation classification \t {",".join(args)}. 1. schema: <relation type> 2. {exp_text}. 3. example: {exp_text} relation classification \t {exp_args[0]},{exp_args[1]} answer: {exp_entity_type}')
                    except:
                        question_list.append(f'relation classification \t {",".join(args)}. 1. schema: <relation type> 2. {exp_text}.')
        ...
        for key,value in item['e_trig_dict'].items():
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    candidates = res_dict['e_role_set']
                    exp_text = ' choices : ' + ','.join(candidates)
                    question_list.append(f'event extraction \t {trigger}. 1. schema: <relation type> 2. {exp_text}. 3. example: {exp_text} event extraction \t {exp_trigger} answer: {",".join(exp_event_struc)}')
    ...
    for text,label in zip(abstracts,labels):
        ...
        if (explanation==0):
            ...
            if (explanation_type=='choice'):
                ...
                question_list.append('paragraph classification.  1. schema: <yes> or <no> 2. choices: yes,no. 3. example: text: {} paragraph classification, answer: {}'.format(exp_sent, exp_label))
                #import pdb; pdb.set_trace()
        ...
        for rel in j['relationMentions']:
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    exp_text = ' choices : ' + ','.join(candidates)
                    try:
                        question_list.append(f'relation classification \t {",".join(args)}.  1. schema: <relation type> 2. choices {exp_text}. 3. example: text: {exp_sent} relation classification \t {exp_args[0]},{exp_args[1]} answer: {exp_answer}')
                    except:
                        question_list.append(f'relation classification \t {",".join(args)}.  1. schema: <relation type> 2. choices {exp_text}.')
        ...
        for ann in j['annotations']:
            ...
            if (explanation==0):
                ...
                if (explanation_type=='choice'):
                    ...
                    question_list.append(f'synthesis action retrieval \t {token}.  1. schema: <action> 2. choices {exp_text}. 3. example: text: {exp_sent} synthesis action retrieval \t {exp_token} answer: {exp_answer}')
    ...
    for sent,label in zip(sents,labels):
        ...
        if (explanation==0):
            ...
            if (explanation_type=='choice'):
                ...
                question_list.append('sentence classification.  1. schema: <yes> or <no> 2. choices: yes,no. 3. example: text: {} sentence classification, answer: {}'.format(exp_sent, exp_label))
...
def build_dataloader(max_length = 512, batch_size = 4, tasks = [0,1,2,3,4,5,6], explanation = 0, setting = 'ood', train_size = 0.25, base_model = 'matscibert', explanation_type = 'choice', even_split = False):
    ...
    lower = True
    if (base_model=='matscibert' or 'das' or 'ourmatscibert_base' or 'our_masked_matscibert' or 'difference_masking'or 'our_masked_matscibert_0610'):
        tokenizer = AutoTokenizer.from_pretrained("m3rg-iitd/matscibert")
    elif (base_model=='matbert'):
        # can be download from https://github.com/lbnlp/MatBERT
        tokenizer = AutoTokenizer.from_pretrained("/home/user25/Work_Space/Material_aware_LM/NLP4MatSci-ACL23/matbert-base-uncased")
    ...
    elif (base_model=='basebert'or 'bert' or 'ourbert_base'):
